...
int main()
{
    ...
    */
    ...
    if(testCouleurRouge(red)){
        //TODO: regarder si besoin de mettre 'empty' ou de vider 'red'
        writePicture("Image/pomme-sansRouge.ppm", empty, green, blue);
    }
    else {
        writePicture("Image/pomme-sansRouge.ppm", red, green, blue);
    }
    */
    ...
    cout << "Writing negative picture ; this might take a while (go grab a cup of coffee)..." << endl;
    writePicture("Image/pomme-negative.ppm", red, green, blue);
    cout << "Done !" << endl;
    */
    ...
    // We choose red
    const int SEUIL = 127;
    vector<vector<int>> chosenComposante = red;

    for (int i = 0; i < chosenComposante.size(); i++)
    {
        for (int j = 0; j < chosenComposante[0].size(); j++)
        {
            if(chosenComposante[i][j] > SEUIL){
                red[i][j] = 255;
                green[i][j] = 255;
                blue[i][j] = 255;
            }else{
    ...
    writePicture("Image/pomme-binarisee.ppm", red, green, blue);
    */
